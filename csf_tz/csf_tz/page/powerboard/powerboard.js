...
frappe.pages['powerboard'].on_page_load = function(wrapper) {
	...
	frappe.powerboard = new Dashboard(wrapper);
	$(wrapper).bind('show', function() {
		frappe.powerboard.show();
	});
};
...
class Dashboard {
	...
	show() {
		...
		if (this.route.length > 1) {
			...
			if (frappe.last_dashboard) {
				frappe.set_route('powerboard', frappe.last_dashboard);
			} else {
				// default dashboard
				frappe.db.get_list('Powerboard', {filters: {is_default: 1}}).then(data => {
					if (data && data.length) {
						frappe.set_route('powerboard', data[0].name);
					} else {
						// no default, get the latest one
						frappe.db.get_list('Powerboard', {limit: 1}).then(data => {
							if (data && data.length) {
								frappe.set_route('powerboard', data[0].name);
							} else {
								// create a new dashboard!
								frappe.new_doc('Powerboard');
							}
						});
					}
				});
	...
	show_dashboard(current_dashboard_name) {
		if(this.dashboard_name !== current_dashboard_name) {
			...
			if (!this.dashboard_name.toLowerCase().includes(__('powerboard'))) {
				// ensure dashboard title has "dashboard"
				title = __('{0} Powerboard', [title]);
			}
	...
	get_dashboard_doc() {
		return frappe.model.with_doc('Powerboard', this.dashboard_name);
	}
	...
	set_dropdown() {
		...
		this.page.add_menu_item('Edit...', () => {
			frappe.set_route('Form', 'Powerboard', frappe.dashboard.dashboard_name);
		}, 1);
		...
		frappe.db.get_list("Powerboard").then(dashboards => {
			dashboards.map(dashboard => {
				let name = dashboard.name;
				if(name != this.dashboard_name){
					this.page.add_menu_item(name, () => frappe.set_route("powerboard", name));
				}
			});
		});
	}
